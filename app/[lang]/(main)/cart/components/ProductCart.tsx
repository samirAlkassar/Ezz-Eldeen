...
const ProductCart = ({ cartItem, handleRemoveItem, handleUpdateItem, wishlist, lang}: ProductCartProps) => {
    ...
    return (
        <div key={cartItem?.product?._id} className="w-full bg-white rounded-xl shadow-sm p-2 md:p-4 flex gap-3 md:gap-4 border-gray-100 relative">
            <div onClick={()=>router.push(`/${lang}/products/${cartItem?.product?.slug}`)} className="w-26 sm:w-30 md:w-40 sm:aspect-square bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center cursor-pointer">
                <Image
            ...
            <div className="flex flex-col justify-between flex-1">
                <div>
                    <div className="flex justify-between items-start">
                        <h1 className="text-base md:text-lg truncate max-w-25 sm:max-w-fit overflow-ellipsis font-medium text-gray-800 leading-normal">{cartItem?.product?.name}</h1>
