...
const ProfileAsideTab = ({title, currentProfileSection, setCurrentProfileSection, icon}:
    ...
    return (
        <div
            ...
            className={twMerge("flex items-center justify-center md:justify-start gap-2 px-4 py-3 rounded-lg cursor-pointer font-medium",
                currentProfileSection === title ? "bg-orange-400 hover:opacity-90 text-white" : "bg-transparent hover:bg-gray-100"
            )}>
            {icon}
            <p className="text-base hidden md:block">{title}</p>
        </div>
    )
}
