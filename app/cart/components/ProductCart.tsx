...
const ProductCart = ({ cartItem, handleRemoveItem, handleUpdateItem, wishlist }: { cartItem: CartItem, handleRemoveItem: (id: string) => void, handleUpdateItem: (id: string, quantity: number) => void, wishlist: ProductType[] }) => {
    ...
    return (
        <div key={cartItem?.product?._id} className="w-full bg-white rounded-xl shadow-sm p-4 flex gap-4 border-gray-100 relative">
            <div onClick={()=>router.push(`/products/${cartItem?.product?.slug}`)} className="w-52 aspect-square bg-gray-100 rounded-lg overflow-hidden flex items-center justify-center cursor-pointer">
                <Image
            ...
            <div className="flex flex-col justify-between flex-1">
                <div>
                    <div className="flex justify-between items-start">
                        <h1 className="text-lg font-bold text-gray-800 leading-tight">{cartItem?.product?.name}</h1>

                        <span className="px-3 py-1 bg-purple-100 text-purple-700 font-bold rounded-xl text-base shadow-inner">
                            {cartItem?.product?.price}
                        </span>
                    </div>

                    <div className="flex justify-between gap-4">
                        <p className="text-sm text-gray-500 mt-1 w-full flex-4 line-clamp-2 max-h-10 overflow-clip max-w-xs">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-xl shadow-inner">
                            <button
                                ...
                                disabled={quantity <= 1}
                                className="px-4 py-1 rounded-lg bg-red-200 hover:bg-red-300 text-red-700 font-bold transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                                -
                            </button>
                            ...
                            <button
                                ...
                                className="px-4 py-1 rounded-lg bg-green-200 hover:bg-green-300 text-green-700 font-bold transition disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                                +
                            </button>
                        </div>
                    </div>
                ...
                <div className="flex gap-2 mt-4 justify-end">
                    <button
                        className="bg-red-400 h-10 px-4 gap-1 flex items-center justify-center rounded-lg text-white font-medium cursor-pointer hover:bg-red-500 active:scale-[97%] transition-all duration-75 ease-in"
                        onClick={() => handleRemove(cartItem?.product?._id)}>
                        <p className="font-medium text-white text-base">remove</p>
                        <Trash size={20}/>
                    </button>
                    <button
                        className="flex items-center justify-center relative bg-gray-200 w-10 h-10 rounded-lg text-white font-medium cursor-pointer hover:bg-gray-300 active:scale-[97%] transition-all duration-75 ease-in"
